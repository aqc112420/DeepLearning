...
cost = xent.mean() + 0.01 * (w**2).sum()#损失函数的平均值加上L2正则项
#这里的正则项系数直接固定成0.01
...
train = theano.function(inputs=[x,y],outputs=[prediction,xent],updates=\
    ((w,w-0.1*gw),(b,b-0.1*gb)))#训练所需要的函数
predict = theano.function(inputs=[x],outputs=prediction)#预测阶段函数

#训练
training_steps = 1000
for i in range(training_steps):
    pred,err = train(D[0],D[1])
    print(err.mean())#查看损失函数的下降变化过程
